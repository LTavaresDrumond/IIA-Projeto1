...
df_matriz_utilidade = pd.DataFrame(
    matriz_utilidade, index=indices_usuarios, columns=colunas_jogos
)
...
df_jogos["perfil_conteudo"] = df_jogos[
    [
        "caracteristica_1",
        "caracteristica_2",
        "caracteristica_3",
...
tfidf = (
    TfidfVectorizer()
)  # (stop_words='english')não sera necessário pois as características já estão apenas em palavras chaves
...
print(
    "Forma da matriz TF-IDF (jogos x características únicas):", matriz_tfidf_jogos.shape
)
...
jogos_gostados = avaliacoes_usuario[
    (avaliacoes_usuario >= 4) & (avaliacoes_usuario > 0)
]
...
if not indices_jogos_gostados:
    print(
        f"Usuário {id_usuario_exemplo} não avaliou nenhum jogo com nota 4 ou superior."
    )
    ...
    popularidade_jogos = (
        (df_matriz_utilidade > 0).sum(axis=0).sort_values(ascending=False)
    )
    ...
    df_recomendacoes = pd.DataFrame(
        {"jogo": colunas_jogos, "score": scores_similaridade}
    )

    # Filtra os já avaliados
    df_recomendacoes = df_recomendacoes[
        ~df_recomendacoes["jogo"].isin(jogos_avaliados_pelo_usuario)
    ]
